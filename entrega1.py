...
class ExploracionRobotica(SearchProblem):    
            ...
            if len(elemento) == 3:
                ...
                if bateria >= 100:
                    ...
                    for posicion_posible in posiciones_posibles:
                        if posicion_posible in self.tuneles:
    ...
    def is_goal(self, state):
        ...
        condicion = len(self.tuneles_recorridos) == len(self.tuneles)
        return condicion
    
    def heuristic(self, state):
        # La heurística es la cantidad de túneles que faltan recorrer multiplicados por 1 minuto
        tiempo = [len(self.tuneles) - len(self.tuneles_recorridos)] * 1
        return tiempo
...
def planear_escaneo(tuneles, robots):
    ...
    problem = ExploracionRobotica(estado_tupla)
